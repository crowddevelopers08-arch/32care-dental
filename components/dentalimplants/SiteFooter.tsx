    ...
    <footer className="relative bg-[#102532] px-5 pt-12 font-['Onest',sans-serif] text-white sm:px-8 lg:px-12">
      <div className="mx-auto max-w-[1315px]">
        <div className="grid items-center max-sm:gap-4 gap-8 pb-6 md:grid-cols-[1fr_1.6fr_1fr]">
          <a href="/dental-implant" aria-label="32Care Dental Clinic home" className="relative block h-[76px] w-[230px] rounded-[8px] max-w-full bg-white">
            ...
            <nav className="mt-8 max-sm:mt-3 grid max-sm:gap-2 gap-4 text-[13px] text-[#d5e0e7]">
              {navLinks.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  className="flex items-center gap-3 transition hover:translate-x-1 hover:text-[#54b6ec]"
                >
                  <span className="h-px w-3 bg-[#8fa2ae]" />
                  {link.label}
                </a>
              ))}
            </nav>
