    ...
    <section id="faqs" className="relative overflow-hidden border-y border-[#e4eef5] bg-[linear-gradient(118deg,#2d78bb_0%,#086cae_54%,#083a75_100%)] px-5 py-12 font-['Onest',sans-serif] text-white sm:px-8 lg:px-12 lg:py-16">
      ...
      <div className="relative mx-auto grid max-w-[1360px] items-center gap-14 max-[620px]:gap-6 lg:grid-cols-[730px_1fr] lg:gap-24">
        <div className="relative order-2 mx-auto h-[610px] w-full max-w-[560px] max-[620px]:order-3 max-[620px]:h-[500px]">
          <div className="absolute top-0 left-0 h-[535px] w-[79%] overflow-hidden bg-[#e8f5fc]  max-[620px]:h-[430px] max-[620px]:w-[84%]">
          ...
          <div className="absolute right-0 bottom-0 z-10 h-[348px] w-[63%] overflow-hidden border-[5px] border-white bg-[#e8f5fc]  max-[620px]:h-[285px] max-[620px]:w-[69%]">
          ...
          <div className="absolute top-[33px] -right-6 z-20 flex min-h-[290px] w-[220px] flex-col justify-center rounded-[6px] bg-[#0876b5] px-10 text-white  max-[620px]:top-[62px] max-[620px]:w-[175px] max-[620px]:min-h-[235px] max-[620px]:px-7">
        ...
        <div className="order-1 max-[620px]:contents">
          <div className="max-[620px]:order-1"><SectionLabel light align="start">Frequently Asked Questions</SectionLabel></div>
          <h2 className="mt-2 max-w-[700px] text-[26px] leading-[1.22] tracking-[-.6px] text-white max-[620px]:order-2 max-[620px]:mt-0 max-[620px]:text-[23px]">Questions Patients Commonly Ask About Root Canal Treatment</h2>

          <div className="mt-8 grid grid-cols-2 items-start gap-3 max-[620px]:order-4 max-[620px]:mt-0 max-[620px]:grid-cols-1">
            {features.map((feature, index) => {
              const isOpen = openFaq === index;
              return <article key={feature.title} className={`overflow-hidden rounded-[12px] border transition-colors duration-300 ${index >= 4 && !showAllFaqs ? "max-[620px]:hidden" : ""} ${index === features.length - 1 && features.length % 2 !== 0 ? "col-span-2 w-[calc(50%-6px)] justify-self-center max-[620px]:col-span-1 max-[620px]:w-full" : ""} ${isOpen ? "border-[#54b6ec] bg-[#e8f5fc]" : "border-[#d7e7f1] bg-white"}`}>
                <button type="button" onClick={() => setOpenFaq(isOpen ? -1 : index)} aria-expanded={isOpen} className="flex w-full items-center justify-between gap-5 px-5 py-4 text-left">
                  ...
                  <span className={`relative grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#0876b5] text-white transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`} aria-hidden="true">
          ...
          <a href="#consultation-form" className="mt-10 inline-flex min-h-[54px] items-center justify-center rounded-full bg-[#0876b5] px-8 text-center text-[14px] font-bold text-white  transition duration-300 hover:-translate-y-1 hover:bg-[#073576] max-[620px]:order-6 max-[620px]:mt-0 max-[620px]:w-full max-[620px]:px-3 max-[620px]:text-[12px]"><span className="max-[620px]:hidden">Book Your Root Canal Consultation Today</span><span className="hidden max-[620px]:inline">Book Your Consultation</span></a>
        </div>
      </div>
    </section>
  );
