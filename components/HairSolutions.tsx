...
export function HairSolutions() {
  ...
  return <section id="treatments" className="scroll-mt-5 bg-white px-5 py-5 font-['Onest',sans-serif] text-[#092b4c] sm:px-8 sm:py-8 lg:px-12">
    <div className="mx-auto max-w-[1350px] ">
      <Reveal className="text-center lg:mb-10" direction="right">
      ...
      <div ref={carousel} onScroll={(event) => { const slides = Array.from(event.currentTarget.children) as HTMLElement[]; const nearest = slides.reduce((closest, slide, index) => Math.abs(slide.offsetLeft - event.currentTarget.scrollLeft) < Math.abs(slides[closest].offsetLeft - event.currentTarget.scrollLeft) ? index : closest, 0); setActiveSlide(nearest); }} className="mt-6 grid grid-cols-5 gap-x-5 gap-y-8 max-[1100px]:grid-cols-3 max-[700px]:grid-cols-2 max-[620px]:mt-4 max-[620px]:flex max-[620px]:snap-x max-[620px]:snap-mandatory max-[620px]:gap-0 max-[620px]:overflow-x-scroll max-[620px]:pb-1 max-[620px]:[scrollbar-width:none] max-[620px]:[&::-webkit-scrollbar]:hidden">
        {solutions.map((solution, index) => <Reveal as="article" key={solution.title} delay={(index % 5) * 90} direction={directions[index % directions.length]} className="text-center max-[620px]:w-screen max-[620px]:max-w-full max-[620px]:shrink-0 max-[620px]:basis-full max-[620px]:snap-center max-[620px]:snap-always max-[620px]:px-6">
      ...
      <div className="mt-4 hidden items-center justify-center gap-4 max-[620px]:flex">
        ...
        <div className="flex max-w-[170px] flex-wrap items-center justify-center gap-1.5 max-sm:gap-1.2">
          {solutions.map((solution, index) => <button type="button" key={solution.title} onClick={() => goToSlide(index)} aria-label={`Show treatment ${index + 1}`} className={`h-2.5 rounded-full bg-[#0067ac] transition-all duration-300 ${activeSlide === index ? "w-6" : "w-2.5 opacity-30"}`} />)}
        </div>
      ...
      <Reveal className="mt-6 flex justify-center max-[620px]:mt-5">
        <a href="#consultation-form" className="group relative inline-flex min-h-[48px] items-center justify-center overflow-hidden rounded-full border-2 border-[#073576] px-7 text-center text-[15px] font-bold text-[#073576] transition-all duration-300 hover:-translate-y-1 max-[620px]:min-h-[54px] max-[620px]:w-full max-[620px]:px-3 max-[620px]:text-[12px] max-[620px]:leading-tight">
          ...
          <span className="relative z-10 hidden max-[620px]:inline transition-colors duration-500 group-hover:text-white">Book Your Consultation</span>
        </a>
      </Reveal>
    </div>
  </section>;
