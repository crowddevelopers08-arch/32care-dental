...
export function DentalHero() {
  ...
  const submit = (e: FormEvent<HTMLFormElement>) => { e.preventDefault(); setShowError(phone.length !== 10); };
  ...
  return <main className="relative min-h-screen overflow-hidden bg-[linear-gradient(118deg,#2d78bb_0%,#086cae_54%,#083a75_100%)] pb-[76px] font-sans text-white max-[620px]:pb-[108px] [@media(min-width:1400px)_and_(max-height:900px)]:min-h-[125vh] [@media(min-width:1400px)_and_(max-height:900px)]:[zoom:.8]">
    ...
    <header className="relative z-[4] mx-auto flex h-[108px] max-w-[1660px] items-start justify-between px-7 py-5 max-[900px]:h-auto max-[900px]:items-center max-[620px]:px-[15px] max-[620px]:py-3">
      ...
      <nav className="flex h-[63px] items-center gap-6 whitespace-nowrap text-base max-[620px]:gap-2">
        ...
        <a className="flex h-[60px] items-center gap-3 rounded-[34px] bg-[#073576] px-[34px] text-[21px] font-bold text-white no-underline max-[900px]:h-12 max-[900px]:px-[18px] max-[900px]:text-base max-[620px]:h-11 max-[620px]:w-11 max-[620px]:justify-center max-[620px]:px-0 max-[620px]:text-[length:0px]" href="tel:+919975611589"><Phone />+91 9975611589</a>
      ...
      <div className={`absolute top-full right-[15px] left-[15px] overflow-hidden rounded-[16px] bg-white text-[#092b4c]  transition-all duration-300 min-[901px]:hidden ${mobileNavOpen ? "visible translate-y-0 opacity-100" : "invisible -translate-y-3 opacity-0"}`}>
        <nav className="grid p-3">
          {navLinks.map(([label,href]) => <a key={href} href={href} onClick={() => setMobileNavOpen(false)} className="border-b border-[#dceaf3] px-4 py-3 text-[14px] font-bold transition hover:bg-[#e8f5fc] hover:text-[#0876b5]">{label}</a>)}
        </nav>
    ...
    <div className="relative z-[3] mx-auto mt-[35px] grid max-w-[1660px] grid-cols-[1.25fr_1fr] items-center gap-[60px] px-7 max-[1200px]:grid-cols-2 max-[1200px]:gap-6 max-[900px]:mt-6 max-[900px]:grid-cols-1 max-[620px]:mt-1 max-[620px]:gap-7 max-[620px]:px-[15px]">
      <section className="pt-6 max-[900px]:text-center max-[620px]:contents">
        <div className="mb-5 flex gap-2 max-[900px]:justify-center max-[620px]:order-1 max-[620px]:mb-0">
          <TrustBadge seal="PDS" text=" Personalised Dental Solutions" />
          <TrustBadge seal="EDC" text=" Expert Dental Care" />
        </div>
        <h1 className="m-0 text-[54px] leading-[1.14] font-semibold tracking-[-1.5px] text-[#dceaff] max-[1200px]:text-[42px] max-[620px]:order-2 max-[620px]:text-center max-[620px]:text-[29px] max-[620px]:leading-[1.18] max-[620px]:tracking-[-.5px]">Relieve Tooth Pain &amp;<br className="max-[620px]:hidden" />Restore Your Smile With <strong className="font-extrabold text-white">Advanced Root Canal Care in Pune</strong></h1>
        <p className="mt-[21px] max-w-[800px] text-[23px] leading-[1.56] font-medium max-[1200px]:text-[19px] max-[900px]:mx-auto max-[620px]:order-3 max-[620px]:mt-0 max-[620px]:text-center max-[620px]:text-[15px] max-[620px]:leading-[1.5]">Experience personalised root canal care designed to preserve your natural tooth and restore confident, comfortable smiles</p>
        <div className="mt-6 flex items-center gap-3 text-[20px] font-bold text-white max-[900px]:justify-center max-[620px]:order-5 max-[620px]:mt-0 max-[620px]:gap-2 max-[620px]:text-[15px]"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-white text-sm font-extrabold text-[#0876b5] max-[620px]:h-6 max-[620px]:w-6 max-[620px]:text-[12px]">✓</span>Advanced Treatment Approach</div>
        <a href="#consultation-form" className="group relative mt-6 inline-flex min-h-[60px] animate-pulse items-center justify-center gap-3 overflow-hidden rounded-full bg-[linear-gradient(110deg,#073576_0%,#0876b5_48%,#073576_100%)] bg-[length:220%_100%] px-8 text-[18px] font-extrabold text-white no-underline  ring-2 ring-white/25 transition-all duration-500 before:absolute before:inset-0 before:bg-white/10 before:content-[''] hover:-translate-y-1 hover:bg-right  hover:ring-white/50 active:translate-y-0 motion-reduce:animate-none max-[620px]:order-6 max-[620px]:mt-0 max-[620px]:min-h-[54px] max-[620px]:w-full max-[620px]:animate-none max-[620px]:gap-1.5 max-[620px]:bg-[#073576] max-[620px]:px-3 max-[620px]:text-[12px]  max-[620px]:ring-white/40"><span className="relative z-10 whitespace-nowrap">Book Your Root Canal Consultation</span><span className="relative z-10 animate-bounce text-xl motion-reduce:animate-none max-[620px]:animate-none">→</span></a>
      ...
      <form id="consultation-form" onSubmit={submit} noValidate className="w-[655px] max-w-full justify-self-center rounded-3xl bg-white px-8 pt-11 pb-[30px] text-[#0a2037]  max-[1200px]:px-6 max-[620px]:order-4 max-[620px]:rounded-[18px] max-[620px]:px-[14px] max-[620px]:pt-6 max-[620px]:pb-5">
        ...
        <div className="grid grid-cols-2 gap-x-5 gap-y-6 [&_label]:relative [&_label]:text-lg [&_label]:font-bold max-[620px]:grid-cols-1 max-[620px]:gap-y-4 max-[620px]:[&_label]:text-[14px]">
          <label>Full Name<input className={field} placeholder="Enter your full name" /></label>
          <label>Mobile Number<div className={`mt-0.5 flex h-[67px] items-center rounded-[34px] border-2 bg-[#fbfdfe] px-6 max-[620px]:h-[56px] max-[620px]:rounded-[28px] max-[620px]:px-4 ${showError?'border-[#d92b2e] ':'border-[#c9e6f5]'}`}><b>+91</b><input className="min-w-0 flex-1 bg-transparent pl-[17px] text-[17px] font-semibold outline-none placeholder:text-[#778493] max-[620px]:pl-3 max-[620px]:text-[14px]" value={phone} onChange={e=>{setPhone(e.target.value.replace(/\D/g,'').slice(0,10));setShowError(false)}} inputMode="numeric" placeholder="10-digit number" /></div>{showError && <small className="absolute top-full right-0 left-0 mt-2 block text-center text-[15px] text-[#dd2929] max-[620px]:z-10 max-[620px]:mt-1 max-[620px]:text-[11px]">Please enter your mobile number.</small>}</label>
          <label>Dental Concern<select className={field} defaultValue=""><option value="" disabled>Select your concern</option><option>Root canal treatment</option><option>Tooth pain</option><option>Other</option></select></label>
          <label>Select Your Nearest Branch<select className={field} defaultValue=""><option value="" disabled>Select your nearest Branch</option><option>Kothapet</option><option>Ameerpet</option><option>Attapur</option></select></label>
        </div>
        <button className="mt-6 h-[67px] w-full cursor-pointer rounded-[34px] border-0 bg-[#0876b5] text-lg font-extrabold text-white  max-[620px]:mt-5 max-[620px]:h-[56px] max-[620px]:rounded-[28px] max-[620px]:px-3 max-[620px]:text-[13px] max-[375px]:text-[13px]" type="submit">BOOK YOUR ROOT CANAL CONSULTATION</button>
...
function Stat({end,suffix,label}:{end:number;suffix:string;label:string}) {
  ...
  return <div className="flex min-h-[92px] flex-col items-center justify-center bg-[#c6e5fb] px-3 py-3 text-center max-[620px]:min-h-[78px] max-[620px]:px-1.5 max-[620px]:py-2">
    <strong className="text-[25px] leading-none font-extrabold text-[#0876b5] max-[620px]:text-xl">{value.toLocaleString("en-IN")}{suffix}</strong>
...
const Check = () => <svg className="h-[13px] w-[13px] fill-[#0876b5]" viewBox="0 0 16 16"><path d="M6.2 12.2 2 8l1.4-1.4 2.8 2.8 6.4-6.4L14 4.4Z" /></svg>;
...
function DoubleTreatmentTicker() {
  ...
  return <div onMouseEnter={() => paused.current = true} onMouseLeave={() => paused.current = false} className="absolute inset-x-0 bottom-0 z-[5] flex items-stretch bg-white  max-[620px]:flex-col">
    <div className="flex shrink-0 items-center gap-2 bg-[#0876b5] px-6 py-3 text-[13px] font-extrabold tracking-[.4px] text-white max-[620px]:justify-center max-[620px]:px-4 max-[620px]:py-2">
